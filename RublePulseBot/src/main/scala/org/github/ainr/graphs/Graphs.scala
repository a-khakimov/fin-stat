...
object Graphs {
  ...
  def apply[F[_]: Async: Parallel](): Resource[F, Graphs[F]] = Resource.eval[F, Graphs[F]] {
    Async[F].delay {
      new Graphs[F] {
        override def plot(input: Input): F[File] = for {
          plots <- input.data.parTraverse {
            data => getPlot(data.seq, data.main, data.xlab, data.ylab, data.color)
          }
          sequence <- Async[F].delay(
            nspl.sequence(plots, nspl.TableLayout(plots.size))
          )
          realTime <- Clock[F].realTime
          file <- Async[F].delay(
            awtrenderer.renderToFile(
              f = File.createTempFile(s"nspl_plot_${realTime.toMicros}", ".png"),
              elem = sequence,
              width = 2000,
              mimeType = "image/png"
            )
